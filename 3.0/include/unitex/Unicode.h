...
UNITEX_FUNC int u_feof(U_FILE* stream);
UNITEX_FUNC size_t fread(void *ptr, size_t size, size_t nmemb, U_FILE *stream);
UNITEX_FUNC size_t fwrite(const void *ptr, size_t size, size_t nmemb, U_FILE *stream);
...
UNITEX_FUNC int u_fgets(unichar*, U_FILE*);
UNITEX_FUNC int u_fgets(unichar*, int, U_FILE*);
int u_fgets_limit2(unichar*,int,U_FILE*);
int u_fgets2(unichar*,U_FILE*);
int u_fgets_treat_cr_as_lf(unichar* s,int size,U_FILE* f);

UNITEX_FUNC int u_printf(const char*, ...);
UNITEX_FUNC int u_fprintf(U_FILE*, const char*, ...);
 ...
 *       utf16le-bom
 */
UNITEX_FUNC U_FILE* u_fopen(Encoding, const char*, OpenMode);
UNITEX_FUNC U_FILE* u_fopen(const VersatileEncodingConfig* cfg, const char*, OpenMode);


UNITEX_FUNC int u_fclose(U_FILE*);
...
UNITEX_FUNC int u_ungetc_UTF16LE_raw(ABSTRACTFILE*);
UNITEX_FUNC int u_ungetc_UTF16BE_raw(ABSTRACTFILE*);
UNITEX_FUNC int u_ungetc_UTF8_raw(unichar, ABSTRACTFILE*);
...
UNITEX_FUNC int u_fprintf(U_FILE*, const char*, ...);
UNITEX_FUNC int u_vfprintf(U_FILE*, const char*, va_list);
UNITEX_FUNC int u_vfprintf_conv_lf_to_crlf_option(U_FILE*, int, const char*, va_list);
UNITEX_FUNC int u_sprintf(unichar*, const char*, ...);
UNITEX_FUNC int u_vsprintf(unichar*, const char*, va_list);

int u_fscanf(U_FILE*, const char*, ...);
UNITEX_FUNC int u_vfscanf(U_FILE*, const char*, va_list);
UNITEX_FUNC int u_sscanf(unichar*, const char*, ...);
UNITEX_FUNC int u_vsscanf(unichar*, const char*, va_list);

/* ------------------- String functions ------------------- */
UNITEX_FUNC unsigned int u_strlen(const unichar*);
UNITEX_FUNC unichar* u_strcpy(unichar*, const unichar*);
UNITEX_FUNC unichar* u_strcpy(unichar*, const char*);
UNITEX_FUNC unichar* u_strncpy(unichar *dest, const unichar *src, unsigned int n);
UNITEX_FUNC unichar* u_strcpy_sized(unichar*, size_t, const unichar*);
UNITEX_FUNC unichar* u_strcpy_sized(unichar*, size_t, const char*);
UNITEX_FUNC unichar* u_strcat(unichar*, const unichar*);
UNITEX_FUNC unichar* u_strcat(unichar*, const char*);

int is_str_mono_unichar_string(const unichar*, unichar);
#define is_str_mono_unichar_string(str,c) (((str)!=NULL) && (*(str)==(c)) && (*((str)+1)==0))
UNITEX_FUNC int u_strcmp(const unichar*, const unichar*);
UNITEX_FUNC int u_strcmp(const unichar*, const char*);
int u_strcmp_ignore_case(const unichar*, const unichar*);
int u_strcmp_ignore_case(const unichar*, const char*);
UNITEX_FUNC int u_equal(const unichar*, const unichar*);
int u_equal_ignore_case(const unichar*, const unichar*);
UNITEX_FUNC unichar* u_strdup(const unichar*);
unichar* u_strndup(const unichar*,int);
UNITEX_FUNC unichar* keycopy(unichar*);
UNITEX_FUNC unichar* u_strdup(const unichar*, unsigned int);
UNITEX_FUNC unichar* u_strdup(const char*);
UNITEX_FUNC const unichar* u_strchr(const unichar*, unichar, int);
UNITEX_FUNC const unichar* u_strchr(const unichar*, unichar);
UNITEX_FUNC unichar* u_strchr(unichar*, unichar);
int u_strrchr(const unichar*,unichar);
int u_strrchr(const unichar*,char);
UNITEX_FUNC const char* u_strchr(const char*, unichar);
...
UNITEX_FUNC unichar* u_strdup(const unichar* str, Abstract_allocator prv_alloc);
UNITEX_FUNC unichar* u_strdup(const char* str, Abstract_allocator prv_alloc);
UNITEX_FUNC unichar* u_strdup(const unichar* str, int n, Abstract_allocator prv_alloc);
...
UNITEX_FUNC unsigned int hash_unichar(unichar*);
...
UNITEX_FUNC int u_parse_int(const unichar * str, const unichar ** next = NULL);
